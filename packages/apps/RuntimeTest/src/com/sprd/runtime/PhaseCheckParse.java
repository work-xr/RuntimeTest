...
public class PhaseCheckParse {
        ...
        private boolean RunTimePass = false;
        ...
        public PhaseCheckParse() {
            ...
            if(binder != null)
                Log.d(TAG, "PhaseCheckParse Get The service connect!");
            else
                Log.d(TAG, "PhaseCheckParse connect Error!!");
        }

        private String StationTestedStr(char testSign, char item) {
        ...
        private boolean StationTested(char testSign, char item) {
            if(testSign=='0' && item=='0') return true;
            if(testSign=='0' && item=='1') return false;
            return false;
        }
        ...
        public boolean writeStationTested(int station) {
            try{
                ...
                Log.d(TAG, "writeStationTested data = " + data.readString() + " SUCESS!!");
                data.recycle();
                return true;
            }catch (Exception ex) {
                Log.d(TAG, "writeStationTested Exception " + ex.getMessage());
                return false;
            }
        }
        ...
        public boolean writeStationPass(int station) {
            try{
                ...
                Log.d(TAG, "writeStationPass data = " + data.readString() + " SUCESS!!");
                data.recycle();
                return true;
            }catch (Exception ex) {
                Log.d(TAG, "writeStationPass Exception " + ex.getMessage());
                return false;
            }
        }

        public boolean readRuntimeStationPass() {
            Log.d(TAG, "RunTimePass: " + RunTimePass);
            return RunTimePass;
        }
        ...
        public boolean writeChargeSwitch(int value) {
            try{
                ...
                Log.d(TAG, "writeChargeSwitch data = " + reply.readString() + " SUCESS!!");
                data.recycle();
                return true;
            }catch (Exception ex) {
        ...
        public boolean writeStationFail(int station) {
            try{
                ...
                Log.d(TAG, "writeStationFail data = " + data.readString() + " SUCESS!!");
                data.recycle();
                return true;
            }catch (Exception ex) {
                Log.d(TAG, "writeStationFail Exception " + ex.getMessage());
                return false;
            }
        }
        ...
        public String getPhaseCheck() {
            ...
            try{
                ...
                Log.d(TAG, "getPhaseCheck transact SUCESS!!");
                ...
                for(int i=0; i<str.length; i++) {
                    sb.append(str[i]+":"+StationTestedStr(charSign[charSign.length-i-1], charItem[charItem.length-i-1])+"\n");
                    if( i == 8){
                        Log.d(TAG, "str[8]：" + str[i]);
                        Log.d(TAG, "str[8] result：" + StationTestedStr(charSign[charSign.length-i-1], charItem[charItem.length-i-1]));
                        if("PASS".equals(StationTestedStr(charSign[charSign.length-i-1], charItem[charItem.length-i-1]))){
                            RunTimePass = true;
                        }else{
                            RunTimePass = false;
                        }
                ...
                Log.d(TAG, "huasong Exception " + ex.getMessage());
    ...
    private boolean isStationTestedPass(int station){
        binder = ServiceManager.getService("phasechecknative");

        if(binder != null){
            Log.e(TAG, " lichao Get The service connect!");
        }else{
            Log.e(TAG, " lichao connect Error!!");
        }
        ...
        try{
            ...
            Log.e(TAG, "lichao transact SUCESS!!");
            int testSign = reply.readInt();
            int item = reply.readInt();
            String stationName = reply.readString();
            String []str = stationName.split(Pattern.quote("|"));
            ...
            char[] charItem = strItem.toCharArray();
            StringBuffer sb = new StringBuffer();
            Log.e(TAG, "lichao  strTestSign = " + strTestSign + " strItem = " + strItem);
            for(int i=0; i<str.length; i++) {
                sb.append(str[i]+":"+ "i----->" + i+ "----" +StationTested(charSign[charSign.length-i-1], charItem[charItem.length-i-1])+"\n");
                Log.e(TAG, "lichao charSign[i]: " +"i----->" +i +"----"+ charSign[charSign.length-i-1] + " charItem[i]:" + charItem[charItem.length-i-1]);
            }
            if(StationTested(charSign[charSign.length-station-1], charItem[charItem.length-station-1])){
                Log.i("lichao",str[station]+":"+"station------>"+station+ "--------->true");
                return true;
            }
            Log.e("lichao","get phasecheck sucess!\n" + Integer.toHexString(item)+"\n"+sb.toString());
            Log.e(TAG, "lichao sb " + sb.toString());
            data.recycle();
            reply.recycle();
        }catch (Exception ex) {
            Log.e(TAG, "lichao Exception " + ex.getMessage());
            Log.e("lichao","get phasecheck fail:" + ex.getMessage());
        }
        return false;
    }

    public int testPhaseCheck(int station){
        byte flag = 1;
        int mstation = station;
        //checkFileExist();
        getPhaseCheck();
        /*if(isStationTest(mstation) && isStationPass(mstation)){
            return 1;
        }*/
        if(isStationTestedPass(station)){
            return 1;
        }
